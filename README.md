...
- Bobby Smith:
    ...
    - Age: 13
    - Guardian Pseudonym: uhzmISXl7szUDLVuYNvDVf6jiL3ExwCybtg-KlazHU4
...
The pseudonyms serve three purposes:

1. They provide anonymity; CheckMyAge only shares pseudonyms and age ranges (e.g., 13-17, 18+) with Crackle and Pop.
2. They verify that one person is the guardian of another person&mdash;without revealing the identity of those people.
3. They ensure that one person can only verify one account for each site.

Each person also has a different pseudonym for CheckMyAge, Crackle, and Pop:
...
- The current time is September 12, 2023, 4:54:09 PM MDT.
...
- CheckMyAge generates the following verification request:
    - Request ID: xobD2pey8bM2V9mGlH9uskbzX3xMNxRR4m6wcZMWwkI
    - Site: Pop
    - Expiration: September 12, 2023, 4:59:09 PM MDT
- [#1] CheckMyAge stores a copy of the verification request.
- CheckMyAge sends the verification request back to Pop.
- [#2] Pop links the request ID (xobD2pey...) to "publius-jr".
- Pop opens the following URL in a new window on Bobby Smith's browser:
  https://www.checkmyage.com/verify/xobD2pey8bM2V9mGlH9uskbzX3xMNxRR4m6wcZMWwkI

Part II
- CheckMyAge receives a web request at the following URL:
  https://www.checkmyage.com/verify/xobD2pey8bM2V9mGlH9uskbzX3xMNxRR4m6wcZMWwkI
- CheckMyAge gets the verification request ID (xobD2pey...) from the URL.
...
- CheckMyAge links the request ID (xobD2pey...) to Bobby Smith.
...
- CheckMyAge changes the pseudonyms using its secret key for Pop.
- Updated user data:
    - Pseudonym: A8y9RGWwLiwhZSaX0i_TZhyX-2r9DxMmrrngoADCUhE
    - Age: 13-17
    - Guardian Pseudonym: iaDG-BXou0kKr5gg2j0BJj0RKsa00bVvnpbRCiEism4
...
- [#2] Pop matches the request ID (xobD2pey...) to "publius-jr".
- Pop extracts the user data from the age certificate:
    - Pseudonym: A8y9RGWwLiwhZSaX0i_TZhyX-2r9DxMmrrngoADCUhE
    - Age: 13-17
    - Guardian Pseudonym: iaDG-BXou0kKr5gg2j0BJj0RKsa00bVvnpbRCiEism4
- Pop changes the pseudonyms using a secret key.
- Updated user data:
    - Pseudonym: 5L3siUtZ5-HAKlxMGjwvEjFB49FoLJdSpytsuE6WeQU
    - Age: 13-17
    - Guardian Pseudonym: Uum6yHO7tgND6ffCHsidSpghQz8Eq7PlkmWHzkVL2DE
- Pop checks that no other accounts have the same pseudonym (5L3siUtZ...).
...
- The age certificate contains an ephemeral pseudonym.
    - CheckMyAge never sees the pseudonym stored on Pop (`5L3siUtZ...`).
    - Pop never sees the pseudonym stored on CheckMyAge (`KB0b9pDo...`).
